  ...
  <body>
		<form id="widgetForm" class="main">
			...
			<div class="form-group">
				...
				<textarea class="form-control" id="intro" name="intro" placeholder="类型名称说明" placeholder="最多200个字符，一个汉字算2个字符" len="200" tip="最多200个字符，一个汉字算2个字符"></textarea>
			</div>
  		...
  		autoCheckForm("widgetForm");
			...
			$("#submit").on("click",function(){
				...
				$.ajax({
					dataType: "json",
					//type: "POST",
					url: "${pageContext.request.contextPath}/manager/widgetType/creatWidgetTypeInfo",
					data:paramsObj,
					cache: false,
					success: function(res) {
						if(res.success){
							window.parent.layer.alert("新增控件类型成功！",function(i){
								window.parent.layer.closeAll();
								window.parent.frames["mainFrame"].query();
								window.parent.layer.close(window.parent.winList.pop());
							});
						}else{
							window.parent.layer.alert("新增控件类型失败！"+res.message);
						}
					},
					error: function() {
						window.parent.layer.alert("新增控件类型失败！");
					}
				});
			});
  	});
